...
const DashboardPage: React.FC = () => {
    ...
    const handleConvert = async () => {
        ...
        if (!file) { setError('Please select a file to convert.'); return; }
        if (!isAuthenticated) { setError('Authentication error. Please log in again.'); navigate('/login'); return; }
                                        ...
                                        <tr key={conv.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="py-4 px-4 whitespace-nowrap text-sm font-medium">
                                                {conv.status === 'completed' && conv.converted_filename ? (
                                                    <button onClick={() => handleDownload(conv.id)} className="text-blue-600 hover:text-blue-900">
                                                        Download
                                                    </button>
